...
const TOPICS = [
  {
    ...
    studyContent: [
      ...
      { heading: "Current & Leeway", body: "Current and leeway are course corrections — they affect where your boat actually goes, unlike CDMVT which only converts between compass north and true north.\n\nThe full planning chain from chart to compass:\nCOG (desired track on chart) → subtract current → CTW (course through water) → subtract leeway → CTS (course to steer, true) → apply variation → CM (magnetic) → apply deviation → CC (compass course you steer)\n\nThe current triangle is a key exam skill:\n1. Draw your desired track (A to B) on the chart — this is your COG\n2. At B (destination), draw the current vector: direction (set) and distance (drift × time)\n3. This shows where you would end up without correction\n4. From A, draw your boat speed vector to meet the corrected track — this gives your CTW\n5. Apply leeway (toward the wind) to get CTS\n6. Then convert CTS through CDMVT to get your compass course (CC)\n\nLeeway (afdrift from wind): The wind pushes the boat sideways. Expressed as an angle. If the wind is from starboard, leeway pushes you to port — so you correct toward the wind (to starboard).\n\nKey exam trap: Drawing the current triangle the wrong way. Current is applied at the destination to show where you'd end up without correction." },
